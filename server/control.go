...
func (ctl *Control) Start() {
	loginRespMsg := &msg.LoginResp{
		...
		ProxyConfig: util.GetProxyConfigs(ctl.loginMsg.User),
		Error :         "",
	}
...
func (ctl *Control) writer() {
	...
	encWriter, err := crypto.NewWriter(ctl.conn, []byte(g.GlbServerCfg.Encryption))
	if err != nil {
...
func (ctl *Control) reader() {
	...
	encReader := crypto.NewReader(ctl.conn, []byte(g.GlbServerCfg.Encryption))
	for {
